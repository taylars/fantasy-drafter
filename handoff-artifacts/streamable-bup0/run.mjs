...
for(let seed=1;seed<=8;seed++) {
 ...
 await writeFile(`${root}/progress.json`,JSON.stringify({perSeed},null,2));
}
await writeFile(`${root}/results.json`,JSON.stringify({seconds:(performance.now()-started)/1000,train:gradeRuns(runs.slice(0,96)),perSeed,points:runs.map(r=>({seed:r.seed,seat:r.heroSeat,points:r.results[r.heroSeat-1].total}))},null,2));
