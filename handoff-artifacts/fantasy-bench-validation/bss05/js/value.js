...
// today's best lineup: he can be dropped in October at almost no cost, so his
// favorable tail is worth a little and his unfavorable one is worth nothing.
//
// The size of that option is deliberately tiny, and it is measured in
// above-the-wire production rather than in grades. It used to add a flat 10
// season points per point of upside grade on top, which was wrong twice over.
// Upside is already priced, once, inside `adjusted` — the bonus was the same
// correction charged a second time at fifty times the weight. And once the
// starting lineup is filled `gain` is ~0 for everybody, so a term worth 10 or
// 20 points decided the entire tail of the draft by itself: the board took the
// same three or four graded names in rounds 12-15 from every draft slot, and
// three tight ends a team, because nothing capped how many bodies at one
// position could collect it. Deleting it is worth 108 points a season.
 ...
 * receive it.
 *
 * It is worth a couple of points at most, and that is on purpose. Its whole job
 * is to order players `gain` has already flattened to zero, and it does that
 * job by preferring the one who actually outproduces the wire. Anything larger
 * stops being a tiebreaker and starts being the pick.
 */
export function optionValue(player, roster, slots, base, filled = null) {
  ...
  return (BENCH_OPTION_WEIGHT + 0.05 * Math.max(0, player.upside)) * Math.max(0, adjusted(player) - (base[player.position] ?? 0));
}
